...
#define	SR9700_AUTOMAC
#define	MII_BUG_FIX

#ifdef  SR9700_AUTOMAC
/* Global variables for file-based MAC address Machenism */
int mac_used[129] = {0};
int dev_addr[129] = {0};
DEFINE_SPINLOCK(sr9700_lock);
#endif
...
static int sr9700_android_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	{
		spin_lock(&sr9700_lock);
		for(i=0; i<129; i++)
		{
			if(mac_used[i] == 0)
			{
				fp = filp_open("/lib/modules/sr9700_MAC_address", O_RDONLY, 0); 

				if (IS_ERR(fp)) 
				{
					netdev_err(dev->net, "open file error");
					fp = filp_open("/lib/modules/sr9700_MAC_address", O_RDWR | O_CREAT, 0644); 
					if (IS_ERR(fp)) 
					{
		...
		spin_unlock(&sr9700_lock);
		/* Set the MAC address */
		sr_write_async(dev, PAR, 6, (u8 *)(dev->net->dev_addr));
	}
...
static void sr9700_android_unbind(struct usbnet *dev, struct usb_interface *intf)
{
#ifdef	SR9700_AUTOMAC
	int i = 0;
	
    /* stop MAC operation */
	sr_write_reg_async(dev, RCR, 0x00);

    /* Power down and reset PHY */
	sr_write_reg(dev, PRR, 1);
	mdelay(20);		// at least 10ms, here 20ms for safe
	sr_write_reg(dev, PRR, 0);
	udelay(2 * 1000);	// at least 1ms, here 2ms for reading right register
...
static int sr9700_android_rx_fixup(struct usbnet *dev, struct sk_buff *skb)
{
	...
	while (skb->len > SR_RX_OVERHEAD)
	{
		...
		if (sr_skb) 
		{
			...
			sr_skb->tail = sr_skb->data + sr_skb->len;
